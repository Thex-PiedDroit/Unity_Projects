	...
	SubShader 
	{
		...
		Pass
		{
			...
			GS_INPUT VertShader (appdata_t v)
			{
				...
				//ampl in [0.5;1]
				output.ampl = half2((dot(  _WorldSpaceCameraPos - output.pos,output.normal)>=0)*_BackFaceAttenuation+(1-_BackFaceAttenuation),0);
			...
			{					
				half3 vect01 = (p[1].pos - p[0].pos)*_FrameSharpness;
				half3 vect12 = (p[2].pos - p[1].pos)*_FrameSharpness;
				half3 vect20 = (p[0].pos - p[2].pos)*_FrameSharpness;
				half3 vect10 = -vect01;
				half3 vect21 = -vect12;
				half3 vect02 = -vect20;
				
				half3 ZComp =distance( _WorldSpaceCameraPos , ((p[0].pos + p[1].pos + p[2].pos) / 3));
				half effWireframeSize = _LineWidth * (ZComp * 0.1 * (1-_LineWidthDistanceDependent) + _LineWidthDistanceDependent);
				
				//draw wireframe 
				half3 currentTangent = normalize(cross((p[0].pos - p[1].pos), p[0].normal));
				half4 v[27]; //Wireframe Vertices
				half uMap[27]; //Useful to map the color gradient to Vertices
				v[0] = p[0].pos;
				v[1] = half4(p[0].pos + effWireframeSize * currentTangent+vect01, 1.0f);
				v[2] = p[1].pos;
				
				v[18] = v[1];
				v[19] = half4(p[1].pos + effWireframeSize * currentTangent+vect10, 1.0f);
				v[20] = v[2];
				...
				v[3] = p[1].pos;
				v[4] = half4(p[1].pos + effWireframeSize * currentTangent+vect12, 1.0f);
				v[5] = p[2].pos;
				
				v[21] = v[4];
				v[22] = half4(p[2].pos + effWireframeSize * currentTangent+vect21, 1.0f);
				v[23] = v[5];
				...
				v[6] = p[2].pos;
				v[7] = half4(p[2].pos + effWireframeSize * currentTangent+vect20, 1.0f);
				v[8] = p[0].pos;
				
				v[24] = v[7];
				v[25] = half4(p[0].pos + effWireframeSize * currentTangent+vect02, 1.0f);
				v[26] = v[8];
				...
				half4x4 vp = mul(UNITY_MATRIX_MVP, _World2Object);
				FS_INPUT pIn;
				...
				for(idx = 0;idx<9;idx++)
				{
					...
					pIn.uv = half2(uMap[idx],p[0].ampl.x);
			...
			half4 FragShader(FS_INPUT i) :COLOR 
			{
				half4 col = tex2D(_GradientTex, half2(i.uv.x,0));
				half ampl = i.uv.y;
				half4 finalColor = half4(_WireframeColor.r*(col.r*_ActivateGradient-_ActivateGradient+1),_WireframeColor.g*(col.g*_ActivateGradient-_ActivateGradient+1),_WireframeColor.b*(col.b*_ActivateGradient-_ActivateGradient+1),_WireframeColor.a*(col.a*_ActivateGradient-_ActivateGradient+1));
